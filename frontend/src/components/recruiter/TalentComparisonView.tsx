...
function ComparisonSide({ profile }: { profile: TalentProfile }) { const [fileError, setFileError] = useState<string | null>(null); return <Paper component="article" elevation={0} sx={{ p: { xs: 2, sm: 2.5 }, border: 1, borderColor: "divider" }}><Stack spacing={2.25}>
  ...
  <CompareSection title="Sobre"><Typography sx={{ whiteSpace: "pre-wrap" }} color={profile.bio ? "text.primary" : "text.secondary"}>{profile.bio || "Apresentação profissional não informada."}</Typography></CompareSection>
  ...
  <CompareSection title="Projetos">{profile.projetos.length ? <Stack spacing={1}>{profile.projetos.map((item) => <Box key={item.id}><Typography variant="subtitle2">{item.nome}</Typography><Typography variant="body2">{item.descricao}</Typography><Stack direction="row" spacing={1}>{item.demoUrl && <Button component="a" href={item.demoUrl} target="_blank" rel="noreferrer" size="small">Demonstração</Button>}{item.repositorioUrl && <Button component="a" href={item.repositorioUrl} target="_blank" rel="noreferrer" size="small">Repositório</Button>}</Stack></Box>)}</Stack> : <Empty />}</CompareSection>
  ...
  <CompareSection title="Contato"><Stack spacing={.5}>{profile.contato.emailProfissional && <Button component="a" href={`mailto:${profile.contato.emailProfissional}`}>E-mail</Button>}{profile.contato.telefone && <Button component="a" href={`tel:${profile.contato.telefone}`}>Telefone</Button>}{profile.contato.linkedInUrl && <Button component="a" href={profile.contato.linkedInUrl} target="_blank" rel="noreferrer">LinkedIn</Button>}{profile.contato.gitHubUrl && <Button component="a" href={profile.contato.gitHubUrl} target="_blank" rel="noreferrer">GitHub</Button>}{profile.contato.portfolioUrl && <Button component="a" href={profile.contato.portfolioUrl} target="_blank" rel="noreferrer">Portfólio</Button>}{profile.curriculo.possuiCurriculo && profile.curriculo.url && <ProtectedFileButton path={profile.curriculo.url} label="Abrir currículo" onError={setFileError} />}</Stack></CompareSection>
</Stack></Paper>; }
function Common({ common }: { common: TalentCommon }) { return <Paper component="section" elevation={0} sx={{ p: { xs: 2, sm: 2.5 }, border: 1, borderColor: "primary.light", bgcolor: "action.hover" }}><Stack spacing={2.5}><Typography component="h2" variant="h6" align="center">Em comum</Typography><CompareSection title="Competências gerais"><Chips values={common.competencias.map((item) => item.nome)} emptyText="Nenhum item em comum." /></CompareSection><CompareSection title="Disponibilidade"><Chips values={common.disponibilidades.map((item) => DISPONIBILIDADE_LABELS[item])} emptyText="Nenhum item em comum." /></CompareSection><CompareSection title="Modalidades"><Chips values={common.modalidades.map((item) => MODALIDADE_LABELS[item])} emptyText="Nenhum item em comum." /></CompareSection></Stack></Paper>; }
