...
export function FavoritesView() {
  ...
  const navigate = useCallback((nextPage: number) => router.push(`${pathname}${nextPage > 1 ? `?page=${nextPage}` : ""}`), [pathname, router]);
  ...
  useEffect(() => { let active = true; setLoading(true); setError(null);
    fetchFavorites(page).then((value) => { if (active) setData(value); }).catch((reason) => { if (active) setError(getApiErrorMessage(reason, "NÃ£o foi possÃ­vel carregar seus favoritos.")); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [page, version]);
  function retry() { setVersion((value) => value + 1); }
  ...
  return <Container maxWidth="xl" sx={{ py: { xs: 3, md: 4 } }}><Stack spacing={2.5}>
    ...
    {loading ? [1, 2, 3].map((item) => <Skeleton key={item} variant="rounded" height={190} />) : data?.items.length ? <>
      {data.items.map(({ talento }) => <TalentCard key={talento.id} talent={talento} onFavoriteChange={(favorite) => favoriteChanged(talento.slug, favorite)} comparisonSelected={selected.some((item) => item.id === talento.id)} onComparisonChange={(checked) => selectionChanged(talento, checked)} />)}
